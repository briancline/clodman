...
A combination of the uid/gid in the Containerfile (lines 75-76) and running the container with `--userns=keep-id` help keep file permissions problems at bay without subuid/subgid worries. On Fedora/RHEL/etc distros, the `:z` option on the mounts helps get around common SELinux problems with container bind mounts too.
